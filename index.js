	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '❌ This command can only be used within groups, Baka! ❌',
			...
			const ownerNumber = ["212770565333@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'sticker':
					...
					break
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, 'Which language code is it, uncle?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Where\'s the text, uncle', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('Most of the text is uncle')
					...
					break
				case 'meme':
					meme = await fetchJson('https://meme-api.herokuapp.com/gimme/wholesomeanimemes', { method: 'get' })
				...
				case 'setprefix':
					...
					break
				case 'hilih':
					if (args.length < 1) return reply('Where\'s the text, Baka?')
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use the country code, Baka!')
					try {
				...
				reply(mess.only.aboutbot)
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ Reply a sticker, Baka! ❌')
					reply(mess.wait)
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Already active, Baka!')
						welkom.push(from)
